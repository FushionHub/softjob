...
export function getAppUrl(customBase) {
    if (customBase && !customBase.includes('localhost') && !customBase.includes('127.0.0.1')) {
...
export async function sendVerificationEmail(email, name, token, customBase) {
    const base = getAppUrl(customBase);
...
export async function sendWelcomeEmail(email, name, customBase) {
    const base = getAppUrl(customBase);
    const html = wrapEmail({
        ...
        button: { text: 'Go to Dashboard', url: `${base}/dashboard`, color: '#ef4d45' }
    });
