...
export async function query(sqlQuery, params = []) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            lastError = error;
            const isTimeoutOrDns =
                String(error?.message || '').includes('fetch failed') ||
                String(error?.message || '').includes('timeout') ||
                String(error?.message || '').includes('EAI_AGAIN') ||
                String(error?.sourceError?.code || '').includes('TIMEOUT') ||
                String(error?.sourceError?.cause?.code || '').includes('EAI_AGAIN');
            ...
            if (isMissingRelationOrCol && attempt === 1) {
                try {
                    await ensureSchema();
                    continue;
                } catch (retryError) {
                    logger.error('database schema auto-ensure failed', { code: retryError?.code });
                }
            }

            if (isTimeoutOrDns && attempt < 3) {
                logger.warn('database connection retry', { attempt, reason: error?.message });
    ...
    logger.error('database query error', { code: lastError?.code, sql: String(sqlQuery).slice(0, 160) });
    throw lastError;
}
