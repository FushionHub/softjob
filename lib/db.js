...
// Dual-driver support: postgres:// (Neon, default — original behavior,
// byte-identical) or mysql:// (shared database with the Laravel app).
// The switch is purely the DATABASE_URL scheme. No UI or API logic changes.
...
let pool = null;
let txConn = null; // held pooled connection between BEGIN and COMMIT/ROLLBACK

async function getPool() {
    if (!pool) {
        // Lazy import so the postgres path never requires mysql2 installed.
        const mysql = await import('mysql2/promise');
        const createPool = mysql.createPool || (mysql.default && mysql.default.createPool);
        if (!createPool) throw new Error('mysql2/promise could not be loaded. Run: npm install mysql2');
        pool = createPool({
...
// Remap $1,$2.. to ? keeping values aligned even if numbers repeat/reorder.
export function remapPlaceholders(sqlText, params) {
    const ordered = [];
    const out = String(sqlText).replace(/\$(\d+)/g, (m, n) => {
...
export function splitReturning(sqlText) {
    const m = String(sqlText).match(/^(.*?)\s+RETURNING\s+(.+?)\s*;?\s*$/is);
    if (!m) return null;
    return { head: m[1].trim(), cols: m[2].trim() };
}

function insertTable(head) {
    const m = head.match(/^\s*INSERT\s+INTO\s+([`"\w.]+)/i);
    return m ? m[1].replace(/[`"]/g, '') : null;
}

function updateTable(head) {
    const m = head.match(/^\s*UPDATE\s+([`"\w.]+)/i);
    return m ? m[1].replace(/[`"]/g, '') : null;
}

async function mysqlQuery(sqlText, params = []) {
    const trimmed = String(sqlText).trim();
    const upper = trimmed.toUpperCase();
    if (upper === 'BEGIN' || upper === 'START TRANSACTION') {
        if (!txConn) txConn = await (await getPool()).getConnection();
        await txConn.beginTransaction();
        return [];
    }
    if (upper === 'COMMIT') {
        if (txConn) { await txConn.commit(); txConn.release(); txConn = null; }
        return [];
    }
    if (upper === 'ROLLBACK') {
        if (txConn) { await txConn.rollback(); txConn.release(); txConn = null; }
        return [];
    }

    const run = txConn
        ? (s, p) => txConn.query(s, p).then(([rows]) => rows)
        : (s, p) => getPool().then((pl) => pl.query(s, p)).then(([rows]) => rows);

    const ret = splitReturning(trimmed);
    if (!ret) {
        const r = remapPlaceholders(trimmed.replace(/\bILIKE\b/g, 'LIKE'), params);
        return run(r.sql, r.params);
    }

    const headUpper = ret.head.trim().toUpperCase();
    if (headUpper.startsWith('INSERT')) {
        const table = insertTable(ret.head);
        if (!table) throw new Error('MySQL mode: cannot parse INSERT...RETURNING table');
        const r = remapPlaceholders(ret.head, params);
        const ok = await run(r.sql, r.params);
        const insertId = ok && ok.insertId;
        if (!insertId) return [];
        return run(`SELECT ${ret.cols} FROM ${table} WHERE id = ?`, [insertId]);
    }
    if (headUpper.startsWith('UPDATE')) {
        const table = updateTable(ret.head);
        const wm = ret.head.match(/\sWHERE\s+([\s\S]+?)\s*;?\s*$/i);
        if (!table || !wm) throw new Error('MySQL mode: cannot parse UPDATE...RETURNING (needs WHERE)');
        const whereNums = [...wm[1].matchAll(/\$(\d+)/g)].map((m) => parseInt(m[1], 10) - 1);
        const whereSql = wm[1].replace(/\$(\d+)/g, '?').replace(/\bILIKE\b/g, 'LIKE');
        const main = remapPlaceholders(ret.head, params);
        await run(main.sql, main.params);
        return run(`SELECT ${ret.cols} FROM ${table} WHERE ${whereSql}`, whereNums.map((i) => params[i]));
    }
    throw new Error('MySQL mode: RETURNING is only emulated for INSERT/UPDATE');
}
...
export async function ensureSchema() {
    ...
    // In MySQL mode the Laravel migrations own the schema (Postgres DDL
    // below is not MySQL-compatible). Skip rather than error.
    ...
    isMigrating = (async () => {
        ...
        try {
            ...
            await executeStmt(`ALTER TABLE deposits ADD COLUMN IF NOT EXISTS plan_id INTEGER DEFAULT NULL`);
            ...
            // Seed the configured admin account exactly once. DO NOTHING on
            // conflict — never reset an existing admin's password silently.
            const adminEmail = (process.env.ADMIN_EMAIL || 'jmauricennadi@gmail.com').trim().toLowerCase();
            const adminExists = await db('SELECT id FROM admin_users WHERE LOWER(email)=$1 LIMIT 1', [adminEmail]);
            ...
            const defaultSettings = [
                ...
                ['site_url', 'https://emporiumcapitals.com', 'text', 'general', 'Canonical website URL'],
                ['site_logo', '/assets/logo.png', 'text', 'general', 'Logo URL'],
                ['site_favicon', '/favicon.ico', 'text', 'general', 'Favicon URL'],
                ...
                ['support_phone', '+1 (800) 555-0199', 'text', 'general', 'Support phone number'],
                ['company_address', '100 Wall Street, 24th Floor, New York, NY 10005', 'text', 'general', 'Company physical address'],
                ['meta_title', 'Emporium Capitals | Premium Crypto Investment & Trading Platform', 'text', 'seo', 'Default SEO meta title'],
                ['meta_description', 'Start your wealth-building journey with AI-driven trading strategies, high-yield investment packages, and enterprise-grade asset protection.', 'text', 'seo', 'Default SEO meta description'],
                ['meta_keywords', 'crypto, bitcoin, trading, investment, forex, ai trading, passive income, emporium capitals', 'text', 'seo', 'SEO meta keywords'],
                ['og_image', '/assets/logo.png', 'text', 'seo', 'OpenGraph preview image URL'],
                ['disclaimer_footer', 'Trading cryptocurrencies, forex, and leveraged financial assets involves substantial risk and may lead to the loss of your invested capital. Past performance is not indicative of future results. Never invest capital you cannot afford to lose.', 'text', 'compliance', 'Website footer risk disclaimer'],
                ['disclaimer_email', 'DISCLAIMER: This email and any attachments are intended solely for the recipient. Emporium Capitals will never ask for your password or private recovery phrases via email. Trading digital assets carries significant financial risk.', 'text', 'compliance', 'Email footer disclaimer note'],
                ['disclaimer_trading', 'Risk Notice: Crypto asset prices are highly volatile. Ensure you fully evaluate your risk tolerance before placing trades or allocating capital.', 'text', 'compliance', 'Trading and exchange risk notice'],
            ...
            logger.error('schema auto-ensure error', { message: err?.message });
            return false;
        } finally {
            isMigrating = null;
        }
...
export async function query(sqlQuery, params = []) {
    ...
    if (isMySQL()) {
        try {
            return await database(sqlQuery, params);
        } catch (error) {
            // Log code + SQL shape only — never params (may hold PII).
            logger.error('database query error', { code: error?.code, sql: String(sqlQuery).slice(0, 160) });
            throw error;
        }
    }
    try {
        ...
        if (isMissingRelationOrCol) {
            try {
                ...
                logger.error('database query retry failed', { code: retryError?.code });
                throw retryError;
            }
        }
